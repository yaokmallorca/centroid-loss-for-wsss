...
def retest(testloader, model, use_cuda, criterion, epoch, RESUME):
    ...
    # Save checkpoint.
    if epoch % 10 == 0:
        torch.save(model.state_dict(), 'checkpoint/' + str(RESUME + int(epoch / 10)) + '_crop.pt')
...
def train_roicrop(trainloader, model, use_cuda, epoch, criterion, optimizer):
    model.train()
    correct, total = 0, 0
    acc_sum, loss_sum = 0, 0
    i = 0
...
def cams_roi(dataloader, model, use_cuda, criterion, epoch, RESUME):
    model.eval()
    test_loss = 0
    correct = 0
