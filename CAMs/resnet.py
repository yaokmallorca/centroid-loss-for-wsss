...
def resnet_attention(pretrained=False, model_file=None, **kwargs):
    if pretrained:
        model = ResidualAttentionModel(**kwargs)
        model.load_state_dict((torch.load(model_file)))
        return model

    return ResidualAttentionModel(**kwargs)

class ResidualAttentionModel_AL(nn.Module):
    # for input size 224
    def __init__(self, num_classes):
        super(ResidualAttentionModel_92, self).__init__()
        self.conv1 = nn.Sequential(
    ...
    def forward(self, x):
        ...
        # branch A
        outA = self.residual_block6(out)
        # print("res_block6: ", out.size())
        outA = self.mpool2(outA)
        # print("mpool2: ", out.size())
        outA = outA.view(outA.size(0), -1)
        outA = self.fc_cor(outA)

        # branch A
        outB = self.residual_block6(out)
        # print("res_block6: ", out.size())
        outB = self.mpool2(outB)
        # print("mpool2: ", out.size())
        outB = outB.view(outB.size(0), -1)
        outB = self.fc_cor(outB)

        return outA, outB

def resnet_attention_al(pretrained=False, model_file=None, **kwargs):
    if pretrained:
        model = ResidualAttentionModel_AL(**kwargs)
        model.load_state_dict((torch.load(model_file)))
        return model

    return ResidualAttentionModel_AL(**kwargs)